use axum::{
    extract::{ws::{Message, WebSocket, WebSocketUpgrade}, Query as AxQuery, State as AxState},
    http::StatusCode,
...
use tokenir::constans::helper::{
    fetch_solana_price, get_community_by_id, get_metadata, parse_community_id,
};
...
struct AppState {
    // Ultra-large channel for zero-blocking sends
    ...
    // Separate cache for community IDs
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let database = Arc::new(
        Database::new(std::env::var("SQL").expect("SQL env var missing")).await?
    );
    ...
    tokio::spawn(async move {
        let client = Client::new(url_serving);
        println!("[subscriber] Serving connection started...");
        
        let _ = client.subscribe_to_pump(move |(time, event)| {
            ...
            async move {
                if let Event::Create(data) = event {
                    ...
                    tokio::spawn(async move {
                        if let Some(token) = process_fast_create(data, &tw_key, time, db, cache, comm_cache).await {
                            println!(
                                "[send] {} [twitter] {} [dev] {} [token2022] {}",
                                &token.mint,
    ...
    tokio::spawn(async move {
        ...
        let _ = client.subscribe_to_pump(move |(_time, event)| {
            ...
            async move {
                match event {
                    Event::Create(data) => {
                        tokio::spawn(async move {
                            let _ = process_slow_create(data, db, &tw_key, cache, comm_cache).await;
                        });
                    }
                    Event::Buy(data) => {
...
async fn handle_socket(socket: WebSocket, state: SharedState, key: String, session_id: u64) {
    ...
    let autobuy_status = state.db.get_user_autobuy_status(&key).await.unwrap_or(false);
            ...
            match rx_broadcast.recv().await {
                Ok(arc_msg) => {
                    if sink.send(Message::Text((*arc_msg).clone())).await.is_err() {
                        println!("[ws] send failed for key: {} session: {}", key_clone, session_id);
                        break;
                    }
                }
                ...
                Err(_) => {
                    println!("[ws] broadcast channel closed for key: {} session: {}", key_clone, session_id);
                    break;
                }
            }
        }
    ...
    let recv_task = tokio::spawn(async move {
        while let Some(result) = stream.next().await {
            if result.is_err() {
                println!("[ws] recv error for key: {} session: {}", key_clone2, session_id);
                break;
            }
        }
        println!("[ws] recv stream ended for key: {} session: {}", key_clone2, session_id);
    });
    ...
    if let Some(&current_session_id) = connections.get(&key) {
        if current_session_id == session_id {
            ...
            println!(
                ...
                session_id, key, connections.len()
            );
        } else {
            println!(
...
impl TokenCache {
    fn check_duplicate(
        ...
        if let (Some(desc), Some(n)) = (description, name) {
            if self.desc_name_pairs.contains_key(&(desc.to_string(), n.to_string())) {
                return true;
            }
        }
        ...
        if let (Some(desc), Some(t)) = (description, ticker) {
            if self.desc_ticker_pairs.contains_key(&(desc.to_string(), t.to_string())) {
                return true;
            }
        }
        ...
        if let (Some(n), Some(t)) = (name, ticker) {
            if self.name_ticker_pairs.contains_key(&(n.to_string(), t.to_string())) {
                return true;
            }
        }
    ...
    fn insert_token(
        ...
        if let Some(desc) = description {
            ...
            if let Some(n) = name {
                self.desc_name_pairs.insert((desc.to_string(), n.to_string()), ());
            }
            if let Some(t) = ticker {
                self.desc_ticker_pairs.insert((desc.to_string(), t.to_string()), ());
            }
        }
        if let Some(n) = name {
            self.names.insert(n.to_string(), ());
            if let Some(t) = ticker {
                self.name_ticker_pairs.insert((n.to_string(), t.to_string()), ());
            }
        }
        if let Some(t) = ticker {
...
async fn process_fast_create(
    ...
    let metadata_fut = get_metadata(&data.uri);
    ...
    let (metadata_res, token_exists) = tokio::join!(metadata_fut, db_check_fut);
    ...
    let metadata = match metadata_res {
        ...
        Err(_) => {
            ...
            if cache_guard.check_duplicate(None, Some(&data.uri), None, Some(&data.name), Some(&data.symbol)) {
                return None;
            }
            drop(cache_guard);
            
            return Some(Token::fresh(
                data.name.clone(),
                data.symbol.clone(),
                data.user,
                data.bonding_curve,
    ...
    // OPTIMIZATION: Check cache first (fast path)
    {
        ...
        if cache_guard.check_duplicate(
            ...
            println!("[cache hit] duplicate detected for {}", &data.mint);
            return None;
        }
    }
    ...
    if now.saturating_sub(time) > Duration::from_secs(5) {
        println!("[break early {} took more than 5 seconds to load]", &data.mint);
        return None;
    }

    let community = if let Some(tw) = &metadata.twitter {
        if let Some(id) = parse_community_id(tw) {
            // OPTIMIZATION: Check community cache first
            {
                ...
                if comm_cache_guard.has_community(&id) {
                    println!("[community cache hit] duplicate community {} for {}", &id, &data.mint);
                    return None;
                }
            }
            ...
            if let Ok(comm) = get_community_by_id(twitter_key, &id).await {
                // Add to cache immediately after successful fetch
                let mut comm_cache_guard = comm_cache.lock().await;
                comm_cache_guard.insert_community(&id);
                Some(comm)
            } else {
...
async fn process_slow_create(
    ...
    let metadata = get_metadata(&data.uri).await.ok()?;
    ...
    let token_exists = database.token_any_exists(
        Some(&data.name),
        Some(&data.symbol),
        Some(&data.uri),
        metadata.image.as_deref(),
...
pub async fn median_dev_mcap_excluding(
    ...
    let (median, count) = db.get_dev_median_ath_excluding(&dev_address, exclude_mint)
        .await
        .ok()??;
    let last_three = db.get_last_tokens_by_dev_excluding(&dev_address, exclude_mint, 3)
        .await
        .ok()?;
...
async fn add_user_handler(
    ...
    match state.db.add_user(&req.admin_key, req.payload).await {
        ...
        _ => (StatusCode::FORBIDDEN, Json(serde_json::json!({"error": "unauthorized"}))),
    }
}
...
async fn remove_user_handler(
    ...
    if state.db.remove_user(&req.admin_key, req.user_id).await.is_ok() {
        ...
        return (StatusCode::OK, Json(serde_json::json!({"status": "success"}))).into_response();
    }
...
async fn get_users_handler(
    ...
    match state.db.fetch_all_users(&req.admin_key).await {
        ...
        _ => (StatusCode::FORBIDDEN, Json(serde_json::json!({"error": "forbidden"}))),
    }.into_response()
}
